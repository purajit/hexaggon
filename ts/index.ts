...
interface HexEntry {
  ...
  minihex: SVGPolygonElement;
  hexObject: SVGTextElement;
  x: number;
  y: number;
  c: number;
  r: number;
 ...
 *   togther                                                            *
 ************************************************************************/
type SvgInHtml = SVGSVGElement & HTMLElement;
type SvgGInHtml = SVGGElement & HTMLElement;
type SvgRectInHtml = SVGRectElement & HTMLElement;

// the main drawing
const FILE_NAME_DIV = document.getElementById("fileName");
const SVG = document.getElementById("hexmap") as SvgInHtml;
const MINIMAP = document.getElementById("minimap") as SvgInHtml;
...
const NON_LAYER_CONTROL_SET_PICKER_BUTTONS = document.getElementsByClassName(
  "non-layer-picker-btn",
) as HTMLCollectionOf<HTMLElement>;
const TOOL_PICKER_BUTTONS = document.getElementsByClassName(
  "tool-picker-btn",
) as HTMLCollectionOf<HTMLElement>;
const CONTROL_PANEL_DIVS = document.getElementsByClassName(
  "control-panel",
) as HTMLCollectionOf<HTMLElement>;
const SAVE_BUTTON = document.getElementById("saveBtn");
const FILE_UPLOAD_INPUT = document.getElementById("fileUpload") as HTMLInputElement;
...
// grid layer
const GRID_DIRECTION_BUTTONS = document.getElementsByClassName(
  "grid-direction-btn",
) as HTMLCollectionOf<HTMLElement>;
...
// object layer
const OBJECT_BUTTONS = document.getElementsByClassName(
  "object-btn",
) as HTMLCollectionOf<HTMLElement>;
// text layer
const TEXT_INPUT_DIV = document.getElementById("textInput") as HTMLInputElement;
const TEXT_FONT_SIZE_DIV = document.getElementById("textFontSize") as HTMLInputElement;
...
function dropKeyMouseState(keys: boolean, mouse: boolean) {
  if (keys) {
    ...
    if (GLOBAL_STATE.currentTool == Tools.ZOOM && e.buttons < 3) {
      ...
      if (target.classList.contains("hex")) {
        ...
        handleHexInteraction(
          parseInt(hexAtMouse.getAttribute("c")),
          parseInt(hexAtMouse.getAttribute("r")),
          e.clientX,
          e.clientY,
        );
      }
      ...
      if (target.classList.contains(`eraseable-${GLOBAL_STATE.currentLayer}`)) {
        ...
        if (target.classList.contains("boundary")) {
          ...
          MINIMAP_PREVIEW.querySelectorAll(".miniboundary").forEach((t) => {
            if (
              t.getAttribute("from-crn") == target.getAttribute("from-crn") &&
...
function setPrimaryObject(objectText: string) {
...
function setSecondaryObject(objectText: string) {
...
function setFileBrowserView(fileName: string) {
  ...
  const fileNameDivs = FILE_BROWSER_DIV.getElementsByClassName(
    "loaded-file-name",
  ) as HTMLCollectionOf<HTMLElement>;
    ...
    switch (lastAction.type) {
      ...
      case "boundary": {
        ...
        if (action == "added") {
          ...
          drawBoundaryLine(
            ...
            { c: parseInt(toCRNArr[0]), r: parseInt(toCRNArr[1]), n: parseInt(toCRNArr[2]) },
            target.color,
          );
        }
        break;
      ...
      case "text": {
        ...
        if (action == "added") {
          SVG.querySelectorAll(".in-image-text").forEach((t) => {
            if (
              parseFloat(t.getAttribute("x")) == target.pt.x &&
      ...
      case "path": {
        ...
        if (action == "added") {
          SVG.querySelectorAll(".path-highlight, .path").forEach((e) => {
            if (
              parseInt(e.getAttribute("c1")) == target.fromCR.c &&
              ...
              if (
                (e.classList.contains("path") && e.getAttribute("stroke") == target.lineColor) ||
          ...
          drawLineAndHighlight(
            GLOBAL_STATE.drawing.hexEntries[target.fromCR.c][target.fromCR.r],
            GLOBAL_STATE.drawing.hexEntries[target.toCR.c][target.toCR.r],
            target.lineColor,
 ...
 ****************************/
function setCanvasColor(previousCanvasColor: string, color: string) {
...
function setGridColor(previousGridColor: string, color: string) {
  for (const hexEntriesRow of GLOBAL_STATE.drawing.hexEntries) {
    for (const hexEntry of hexEntriesRow) {
...
function floodFill(
  startC: number,
  startR: number,
  fn: (c: number, r: number, fillColor: string, isFloodFill: boolean) => void,
) {
...
function drawBoundary(e: MouseEvent) {
  ...
  const nextBoundaryPoint =
    GLOBAL_STATE.drawing.hexEntries[closestCRN.c][closestCRN.r].hex.points[closestCRN.n];
  const lineLength =
 ...
 * DRAWING *
 ***********/
function getSvgLayer(layer: string) {
...
function drawHex(c: number, r: number) {
  ...
  const hexEntry: HexEntry = { hex, minihex: null, hexObject: null, x: null, y: null, c, r };
  return hexEntry;
}

function initMiniMap(x: number, y: number, width: number, height: number) {
...
function saveToLocalStorage(fileName: string, svgStr: string, isNew = false) {
